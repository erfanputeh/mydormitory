...
export class RepairPage {
  ...
  constructor(private navCtrl: NavController,private Formrepair:FormBuilder,private repairs:Repair,
    ...
    this.repairs=repairs; //ตัวแปร repairs เป็นตัวแปรที่ส่งข้อมูลยัง providers ของ repair
     ...
     this.http.get("http://192.168.1.4/mydorm/connect/getinfo/get_repair.php?id="+this.id).subscribe(
     (response) => {
       ...
       this.http.get("http://192.168.1.4/mydorm/connect/getinfo/get_material.php?dorm_id="+this.dorm_id).subscribe(
       (response) => {
